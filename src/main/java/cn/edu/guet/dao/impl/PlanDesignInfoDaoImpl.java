...
public class PlanDesignInfoDaoImpl extends BaseDaoImpl<PlanDesignInfo> implements PlanDesignInfoDao {
    ...
    public Page<PlanDesignInfo> searchBill(PlanDesignDTO planDesignDTO) throws SQLException {
        ...
        int count = 1;
        if (!StringUtils.isAllBlank(planDesignDTO.getPlanDesignName())) {
        ...
        if (planDesignDTO.getSpecId() != null) {
            if (whereClause.length() == 0) {
                ...
                where.append(" AND spec_id LIKE ?");
                whereClause = where.toString();
            }
            map.put(count++, planDesignDTO.getSpecId());
        }
        if (!StringUtils.isAllBlank(planDesignDTO.getDesigner())) {
            if (whereClause.length() == 0) {
        ...
        while (rs.next()) {
            ...
            planDesignInfo.setCad_coord_left(String.valueOf(rs.getInt("cad_coord_left")));
            planDesignInfo.setCad_coord_top(String.valueOf(rs.getInt("cad_coord_top")));
            planDesignInfo.setCad_coord_right(String.valueOf(rs.getInt("cad_coord_right")));
            planDesignInfo.setCad_coord_bottom(String.valueOf(rs.getInt("cad_coord_bottom")));
    ...
    @Override
    public Long getPlanDesignIdByPlanBillNo(String planBillNo) throws SQLException {
        // 写JDBC代码
        Connection conn = DBConnection.getConn();
