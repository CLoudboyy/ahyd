...
public class BaseDaoImpl<T> implements BaseDao<T> {
    ...
    public int update(T t) {
        ...
        try {
            while (iter.hasNext()) {
                Method method = iter.next();
                // 拼装预编译SQL语句update update table_name set name=?,email=?,
                if (!list.get(i).getName().equals("getId")) {
                    sql += method.getName().substring(3).toLowerCase() + "=" + "?" + ",";
                }
                if (method.getReturnType().getSimpleName().indexOf("Timestamp") != -1) {
                    obj[i] = Timestamp.valueOf(method.invoke(t, new Object[]{}).toString());
...
//            sql += "?,";
//        }
        // 去掉SQL语句最后一个,符号update table_name set name=?,email=? where id=?
        ...
        try {
            ...
            for (int j = 0; j < obj.length; j++) {
                if (list.get(j).getName().equals("getId")) {
                    pstmt.setObject(obj.length, obj[j]);
                } else {
                    pstmt.setObject(k, obj[j]);
                    k++;
                }
            }
            affectRow = pstmt.executeUpdate();
